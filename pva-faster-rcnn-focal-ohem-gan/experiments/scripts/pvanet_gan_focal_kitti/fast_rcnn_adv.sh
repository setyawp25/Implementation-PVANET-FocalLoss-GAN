...
#  --weights  ./output/${NET}/${TRAIN_IMDB}/pva_gan_focal_kitti__stage1_pretrain_iter_30000.caffemodel \
#  --imdb ${TRAIN_IMDB} \
#  --iters ${ITERS} \
...
#rm -r ./data/KITTI_VOCdevkit/annotations_cache
#  --net ${NET_FINAL} \
for (( iter=$SNAPSHOT_CYCLE; iter<=$ITERS; iter+=$SNAPSHOT_CYCLE ))
    do
        time ./tools/test_net.py --gpu ${GPU_ID} \
        --def ./models/${NET}/stage0_standard/test.prototxt \
        --net ./output/${NET}/${TRAIN_IMDB}/pva_gan_focal_kitti__stage2_adv_iter_${iter}.caffemodel \
        --imdb ${TEST_IMDB} \
        --cfg ./models/${NET}/cfgs/${NET}.yml \
        ${EXTRA_ARGS}
done
