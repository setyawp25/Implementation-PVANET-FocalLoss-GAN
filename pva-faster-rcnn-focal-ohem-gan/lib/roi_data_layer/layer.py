...
class OHEMDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if cfg.TRAIN.BBOX_REG:
            ...
            self._name_to_bottom_map['data'] = 7 # Alice: 2017.11.25 visualization
        else:
            self._name_to_bottom_map['data'] = 4 # Alice: 2017.11.25 visualization
    ...
    def forward(self, bottom, top):
        ...
        #print 'top[rois_hard]_length',  top[0].data
...
class ASDNLossLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        #ORINGINAL: WITHOUT OHEM
        #prop_after  = bottom[3].data

        prop_after  = bottom[3].data[-prop_before.shape[0]:]
...
class ASDNDataLayer(caffe.Layer):
    ...
    def thres_mask_rand(self, mask_pred, labels):
        ...
        #print "pool_len: %d\t sample_num: %d\tmask_pred:" % (pool_len, sample_num), mask_pred.shape
        #print "labels: ", labels
        labels = np.reshape(labels, sample_num)
...
class ROIStackLayer(caffe.Layer):
    """Combine the ROI conv generated by OHEM and GAN"""
    def setup(self, bottom, top):
        """Setup the ROICombineLayer."""
        ...
        self._name_to_bottom_map = {
            'ohem_data': 0,
            'gan_data': 1}
        #print "bottom[0]shape:", bottom[0].shape, "\nbottom[1]shape:", bottom[1].shape
        #assert (bottom[0].shape == bottom[1].shape)

        top[0].reshape(bottom[0].shape[0], bottom[0].shape[1], bottom[0].shape[2], bottom[0].shape[3])

        print 'ROICombineLayer:', top[0].shape

    def forward(self, bottom, top):
        """Compute loss, select RoIs using OHEM. Use RoIs to get blobs and copy them into this layer's top blob vector."""
        ohem_data = bottom[0].data
        gan_data = bottom[1].data

        all_data = np.vstack((ohem_data, gan_data))
        #print "ohem:", ohem_data
        #print "gan:", gan_data
        #print "combined:", all_data

        #print "bottom[0].shape", bottom[0].shape[0], bottom[0].shape[1], bottom[0].shape[2], bottom[0].shape[3]
        #print "bottom[1].shape", bottom[1].shape[0], bottom[1].shape[1], bottom[1].shape[2], bottom[1].shape[3]
        assert (bottom[0].shape[1]==bottom[1].shape[1] and bottom[0].shape[2]==bottom[1].shape[2] and bottom[0].shape[3]==bottom[1].shape[3])
        top[0].reshape(bottom[0].shape[0] + bottom[1].shape[0], bottom[0].shape[1], bottom[0].shape[2], bottom[0].shape[3])
        top[0].data[...] = all_data
